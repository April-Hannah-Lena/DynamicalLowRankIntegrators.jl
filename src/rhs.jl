...
function E(f)
    m, = mass(f)
    ∫dvf = vec(∫dv(f))
    coeffs = x_basis' * x_weight_matrix * (m .- ∫dvf)
...
x_norm(h) = sqrt(h' * x_weight_matrix * h)
v_norm(g) = sqrt(g' * v_weight_matrix * g)

function orthogonal_complement(g, basis, weight_matrix)
    projection = basis' * weight_matrix * g
...
function directional_continuity_error(fₜ₊, fₜ, τ, p)
    ...
    left = v_grid .^ p#orthogonal_complement(v_grid .^ p, v_basis[:, 1:m], v_weight_matrix)
...
function directional_continuity_error(Xₜ₊, Sₜ₊, Vₜ₊, Xₜ, Sₜ, Vₜ, τ, p)
    ...
    x_dependent_error = right * v_weight_matrix * left
...
function norm_continuity_error(Xₜ₊, Sₜ₊, Vₜ₊, Xₜ, Sₜ, Vₜ, τ, p)
    ...
    orth_complement = orthogonal_complement(v_grid .^ p, v_basis[:, 1:m], v_weight_matrix)
    norm_orth_complement = sqrt(orth_complement' * v_weight_matrix * orth_complement)

    forward_diff = ( Xₜ₊ * Sₜ₊ * Vₜ₊'  -  Xₜ * Sₜ * Vₜ' ) / τ
    rhs = RHS_over_f0v(Xₜ, Sₜ, Vₜ)

    x_dependent_error = sqrt.(diag( (forward_diff - rhs) * v_weight_matrix * (forward_diff - rhs)' ))
